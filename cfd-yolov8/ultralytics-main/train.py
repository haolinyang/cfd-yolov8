...
if __name__ == '__main__':
    # Load a model
    model = YOLO('v8/yolov8-EIEStem.yaml')  # 不使用预训练权重训练
    ...
    model.train(
        ...
        batch=16,  # (int) number of images per batch (-1 for AutoBatch)con  16
        ...
        project='yolov8',  # (str, optional) project name
        ...
        degrees=0.0,  # (float) image rotation (+/- deg)  Default:0.0
        translate=0.1,  # (float) image translation (+/- fraction)
        scale=0.5,  # (float) image scale (+/- gain)
        shear=0.0,  # (float) image shear (+/- deg)  Default:0.0
        perspective=0.0,  # (float) image perspective (+/- fraction), range 0-0.001
        flipud=0.0,  # (float) image flip up-down (probability)
        fliplr=0.5,  # (float) image flip left-right (probability)   Default:0.5
        mosaic=1.0,  # (float) image mosaic (probability)
        mixup=0.0,  # (float) image mixup (probability) Default:0.0
